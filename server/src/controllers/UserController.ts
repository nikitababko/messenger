...
class UserController {
  ...
  getMe = (req: express.Request | any, res: express.Response): void => {
    ...
    if (!errors.isEmpty()) {
      ...
      user
        ...
        .then((obj: IUser) => {
          ...
          mailer.sendMail(
            {
              from: "niki.babko@gmail.com",
              to: postData.email,
              subject: "Подтверждение почты Chat",
              html: `Здравствуйте ${postData.fullname}. Для того, чтобы подтвердить почту, перейдите <a href="http://localhost:3000/signup/verify?hash=${obj.confirm_hash}">по этой ссылке</a>`,
            },
